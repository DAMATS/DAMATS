#!/bin/bash
#
# register Landsat combined OLI+TIRS product to EOxServer
#
...
RT="LandsatOLI_TIRS:radiance:uint16"
...
B10="`ls "${BASE_DIR}"/*_B10.{TIF,tif} 2>/dev/null | tail -n 1`"
B11="`ls "${BASE_DIR}"/*_B11.{TIF,tif} 2>/dev/null | tail -n 1`"
...
[ -f "$B10" ] || error "Not a landsat product! Cannot locate band #10 image! B10=$B10"
[ -f "$B11" ] || error "Not a landsat product! Cannot locate band #11 image! B11=$B11"
[ -f "$VRT" ] && rm -fv "$VRT"

# merge bands to a virtual dataset
info "building VRT ... "
[ -f "$VRT" ] && rm -fv "$VRT"
gdalbuildvrt "$VRT" -separate "$B1" "$B2" "$B3" "$B4" "$B5" "$B6" "$B7" "$B9" "$B10" "$B11"
